...
def exact_skull(image, save_path):
    """
    ...
    sitk.WriteImage(skull_stripped_image, save_path)
...
rootpath = "./nii_data/bad"
Resultpath = './processed_data/bad'
# mean, std = get_mean_and_std("./nii_data/good","./nii_data/bad")
for patient in os.listdir(rootpath):
    print(patient)
    image = preprocess_nii(rootpath+"/"+patient)
    image = resampleSize(image, 32)
    save_path = Resultpath + '/' + patient
    image = exact_skull(image, save_path=save_path)
